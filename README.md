...
Redis Key 为 `image-hub:quota:v1:{用户ID}`，不设置过期时间；值通常是剩余次数，上传期间附带图片 ID 作为预占标记。Redisson 看门狗锁让同一账号串行上传，不持有数据库长事务；另一个同时上传的请求返回 `409`，其他账号不受影响。正常额度查询只读 Redis；Key 缺失或上次上传进程中断时，取得锁后根据新版成功图片重建余额。Redis 不可用时拒绝新的上传，恢复后重试。
