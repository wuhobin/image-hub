...
每个账号免费累计总额度由后台“配置管理”维护，初始值为 100，允许设为 0；修改对所有用户生效，已消耗次数不变，剩余最低为 0。成功上传扣一次，失败不扣，删除不返还。前端在整批发送前查询 `GET /api/app/images/quota`；超过余额时整批不发送。后端对每张图片独立校验，耗尽返回业务码 `40301`。多个设备竞争时可能在批次中途停止，已成功的图片保留。

注册成功后尝试在 Redis 初始化累计消耗为 0，不覆盖已有记录；Redis 初始化失败仍返回注册成功，后续普通额度查询沿用恢复逻辑。
...
Redis Key 为 `image-hub:quota:{用户ID}`，不设置过期时间；值为 `u:累计消耗`，上传期间为 `u:包含预占的累计消耗:图片ID`，仅支持这一种格式。Redisson 看门狗锁让同一账号串行上传，不持有数据库长事务；另一个同时上传的请求返回 `409`，其他账号不受影响。正常额度查询读取配置缓存和 Redis 消耗；Key 缺失、损坏或上次上传进程中断时，取得锁后根据新版成功图片恢复消耗。Redis 不可用时拒绝新的上传，恢复后重试。
