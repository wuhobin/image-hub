...
1. 先备份数据库并停止旧实例，不能让不识别 AI 预留积分的旧版与新版混跑。已有数据库执行一次 `deploy/db/migrations/20260915_ai_creation.sql`（需先完成之前的积分、管理后台迁移）；新数据库执行最新 `deploy/db/schema.sql`。应用不自动执行迁移，不要在已有库重跑初始化脚本。
...
已有数据库执行一次 `deploy/db/migrations/20260921_model_points_cost.sql`，为模型、任务及图片记录增加 `points_cost`；新库使用最新 `schema.sql`。迁移前停止旧后端，迁移后启动新版，不能混跑按张数计费的旧版。旧记录默认 1 积分，无需补录历史流水或清理 Redis。后台“模型管理”配置单张费用，前台显示所选模型价格；提交由服务端读取配置并保存快照，后续调价不影响已提交任务。图片与流水在同一事务按快照计费；缓存丢失按图片记录的实际积分总和恢复，包含已删除图片。普通上传仍每张 1 积分。

- 每次生成固定一张；可选择管理员启用的模型、尺寸和质量，按模型的 `pointsCost` 消耗积分，默认 1，管理员可设置为 1–2147483647 的整数。
- 提交先按模型费用预留积分，只有七牛上传和图片记录持久化完成才计入已用；AI 保存不再走普通上传的二次扣额。
- 每位用户同时最多一个未结束任务，由数据库唯一约束和共享积分锁保障；剩余积分仍可用于普通上传。
...
服务中断后，用户查询任务、历史、积分或再次提交时，按当前用户索引检查已超时任务。只有原工作线程不再持有任务锁时，才标记失败并释放积分；不重新调用模型、不读取图片内容、不进行云网络清理。没有用户访问时，遗留任务保持原状态，直到下次访问再处理。排队任务仍由 worker 领取，`AI_WORKER_ENABLED=false` 会暂停生成派发。
...
前端仅在任务进行中每 2 秒查询已知任务详情，直接展示终态图片。历史列表独立加载、每 5
秒重试失败请求，慢列表不阻塞状态轮询；中间状态只更新当前历史行，完成/失败后刷新列表与积分。模型列表仅进入页面或主动重试时加载，不随焦点/任务刷新重复请求。后端公开选项复用父项目二级缓存（实例
`imageHubModels`，键 `image-hub:models:available`，TTL 3 天），只缓存公开 VO，空列表也缓存；Redis
故障时回源数据库。后台新增、编辑、启停和删除沿用系统配置的写前清理、事务提交后失效广播，下次请求重新加载列表；已知缓存故障时拒绝写库。管理分页与任务提交仍直接读库，保证启用校验和积分价格使用真实配置。空闲时停止任务轮询，离开页面清理全部请求和定时器；响应丢失且未知任务
ID 时仍用最近历史确认原提交，不自动重放生成。历史接口按当前页成功任务 ID 一次批量读取本人未删除图片，保留原分页、排序及删除后空图片语义。

`AI 创作耗时` 使用 INFO 记录任务 ID 和中文阶段，耗时统一以秒显示，保留毫秒精度（例如 `50.35 秒`、`0.009 秒`）：`排队等待`（数据库创建时间至认领的近似值，受秒精度和两端时钟误差影响）、`模型请求`（完整模型请求、响应接收与 JSON 解析）、`Base64 解码` 或 `下载图片`、`图片校验`、`上传七牛`、`入库与积分结算`（数据库提交和积分结算）、`总处理`（认领后的总处理时间，包含前述阶段，不能重复相加）。除排队外均使用单调时钟；阶段日志也记录异常退出时的耗时，不代表成功，需结合失败日志与任务终态判断。耗时日志不额外输出请求和响应内容。另有 `OpenAI 请求`、`OpenAI 响应` INFO 日志，以 taskId 关联：请求记录实际发送的 JSON（含提示词和固定的 moderation=low），响应记录 HTTP 状态码及正文，成功响应包含完整 Base64 或图片 URL；错误响应沿用 8KiB 读取上限，超限注明正文截断。请求头与 Authorization 不记录。网络连接失败等未取得响应的情况仍查看完整异常堆栈。
...
`mvn clean verify` 验证原有功能、幂等提交、共享积分、并发竞争、权限隔离、直接上传大图片、失败释放积分、保存事务回滚、一次性云文件补偿、用户访问时处理中断，以及持锁线程不会被误释放。客户端测试只访问本机 HTTP 桩，覆盖 Base64、分块响应和配置边界；不执行付费生图或真实七牛上传。H2 覆盖业务 SQL，已有 MySQL 需先执行删除暂存字段的迁移。
