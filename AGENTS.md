...
文本输入框（包括邀请码、邀请链接等只读复制框）采用无边框样式，默认、悬停及聚焦状态均不显示 `border`、`outline` 或模拟描边的
`box-shadow`。聚焦和校验状态使用清晰的背景色、文字或提示信息区分，保留键盘操作的可见焦点反馈；页面级通用焦点样式不得重新给输入框添加描边。后续新增或修改输入框时均遵循此规范。
